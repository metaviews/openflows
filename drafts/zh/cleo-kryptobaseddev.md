...
信号：Cleo 表现为一个开源 AI 工具项目，发布在公共仓库中。
...
**译注**
1.  **Current (流) vs Currency (流通)**：此处 `Type` 为 `current`，故原文 `current` 译为 `流` 以指代动态的信号；`Currency` 对应 `流通` 指代更宏观的流通层。
2.  **Repository / Openflows**：保留 `GitHub` 品牌名；`Openflows` 首次出现标注 `（开流）` 以强调其“让流通开放”的理路。
3.  **Practitioner (修行者)**：虽原文用 `users`，但在语境中暗示参与实践，故意译为 `实践者` 或保留 `用户` 以保持技术语境的精准。此处保留 `用户` 以确保通用性，但在 `实践载体` 一词中隐含修行者之意。